...
suite('Should get diagnostics for infoset declaration', () => {
	...
	test('Diagnoses infoset declarations with search on unknown type', async () => {
		...
		await testDiagnostics(docUri, [
			{ message: "Type with name 'UnknownType' not found.", range: toRange(2, 5, 2, 51), severity: vscode.DiagnosticSeverity.Error, source: 'ex' },
		]);
	});
	...
	test('Diagnoses infoset declaration with unknown search-column attribute', async () => {
		...
		await testDiagnostics(docUri, [
			{ message: "Attribute 'UnknownAttribute' not found in Type with name 'Type_Test_SearchColumnUnknownAttribute'.", range: toRange(5, 7, 5, 71), severity: vscode.DiagnosticSeverity.Error, source: 'ex' },
		]);
	});
	...
	test('Diagnoses infoset declaration with unknown search-column rule', async () => {
		...
		await testDiagnostics(docUri, [
			{ message: "Rule with name 'Rule_Test_SearchColumnUnknownRule' not found.", range: toRange(6, 7, 6, 121), severity: vscode.DiagnosticSeverity.Error, source: 'ex' },
		]);
	});
