...
suite('Should get diagnostics for infoset call', () => {
	...
	test('Diagnoses infoset call to unknown infoset', async () => {
		...
		await testDiagnostics(docUri, [
			{ message: "Infoset with name 'NotAKnownInfoset' not found.", range: toRange(2, 5, 2, 59), severity: vscode.DiagnosticSeverity.Error, source: 'ex' },
		]);
	});
	...
	test('Diagnoses infoset call with missing mandatory input', async () => {
		...
		await testDiagnostics(docUri, [
			{ message: "Mandatory input 'MandatoryInfosetInput' for Infoset with name 'KnownInfosetWithMandatoryInput' not provided.", range: toRange(2, 5, 2, 73), severity: vscode.DiagnosticSeverity.Error, source: 'ex' },
		]);
	});
