...
suite('Should get diagnostics for ruleloopaction call', () => {
	...
	test('Diagnoses RuleLoopAction call with unknown infoset', async () => {
		...
		await testDiagnostics(docUri, [
			{ message: "Infoset with name 'UnknownInfoset' not found.", range: toRange(2, 37, 2, 53), severity: vscode.DiagnosticSeverity.Error, source: 'ex' },
		]);
	});
	...
	test('Diagnoses RuleLoopAction call with unknown rule', async () => {
		...
		await testDiagnostics(docUri, [
			{ message: "Rule with name 'NotAKnownRule' not found.", range: toRange(2, 61, 2, 76), severity: vscode.DiagnosticSeverity.Error, source: 'ex' },
		]);
	});
