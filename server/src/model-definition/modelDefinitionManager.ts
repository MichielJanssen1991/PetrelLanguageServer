...
import { INodeContext, Definitions } from './symbolsAndReferences';
...
export class ModelDefinitionManager {
	 ...
	 */
	public getModelDefinitionForTag(modelFileContext: ModelFileContext, nodeContext: INodeContext) {
		const definitionsForTag = this.getModelDefinition(modelFileContext)[nodeContext.getCurrentNode().name]||[];
