	...
	protected checkInternal(node: TreeNode): void {
		...
		if (!(node.type==ModelElementTypes.Document || node.tag == ModelElementTypes.Document.toLowerCase())){
			if (tagDefinition){
				...
				this.addMessage(node.range, "STRC00001", `No definition found for tag: '${node.tag}'`);
			}
		}
	...
	private checkChildOccurrences(element: TreeNode, definition: Definition): void {
		...
		element.children.forEach(child => {
			if (Array.isArray(definition.childs) && !definition.childs?.map(x=>x.element?.toLowerCase()).includes(child.tag.toLowerCase())){
				this.addMessage(element.range, "STRC00002", `Invalid child node '${child.tag}' for element '${element.tag}'`);
			}
		});
				...
				switch(x.occurence){
					case "once":
						if (childOccurrences > 1) {
							this.addMessage(element.range, "STRC00003", `Invalid child node occurence '${x.element}' for element '${element.tag}'. Element '${x.element}' could only be applied once to parent '${element.tag}'`);
						} else if (childOccurrences == 0 && x.required){
							this.addMessage(element.range, "STRC00004", `Missing required '${x.element}' for element '${element.tag}'.`);
						}
						break;
					case "at-least-once":
						if (childOccurrences == 0){
							this.addMessage(element.range, "STRC00005", `Invalid child node occurence '${x.element}' for element '${element.tag}'. Element '${x.element}' should be applied at least once to parent '${element.tag}'`);
						}
						break;
				}
			});
	...
	private checkAttributeOccurrences(element: TreeNode, definition: Definition): void {
		...
		definition.attributes.forEach(attr=>{
			...
			if (attr.required && !this.allNodeAttributes.map(x=>x.name.toLowerCase()).includes(attr.name.toLowerCase())){
				this.addMessage(element.range, "STRC00006", `Missing required attribute '${attr.name}' for element '${element.tag}'`);
			} 
					...
					switch (rc.condition) {
						case "==":
							if (
								...
								this.addMessage(element.range, "STRC00007", `Missing required attribute '${attr.name}' for element '${element.tag}'`);
							}
							break;
						default:
							// not implemented yet
			...
			da.validations?.forEach(dv=>{
				if (dv.type == "regex"){
					...
					if (attrElement && !regEx.test(attrElement.value) && !attributeValueIsAVariable(attrElement.value)){
						this.addMessage(element.range, "STRC00008", `Invalid value for '${da.name}': ${dv.message}`);
					}
				}
			});
		});
				...
				switch(da.type?.type){
					case AttributeTypes.Enum:
						if (da.type?.options && !da.type?.options?.map(o=>o.name.toLowerCase()).some(o=>o==attrValue.toLowerCase() || o=="*")){
							this.addMessage(element.range, "STRC00009", `Invalid value for '${da.name}': '${attrValue}' is not a valid option`);
						}
						break;
					case AttributeTypes.Numeric:
						if (!Number(attrValue)) {
							this.addMessage(element.range, "STRC00010", `Invalid value for '${da.name}': '${attrValue}' is not a number`);
						}
						break;
				}
			}
